...
class VaultClient(object):
    ...
    def login(self, vault_addr, auth_method, username=None, password=None, oauth_type=None):
        ...
        if auth_method == "userpass":
            if username is None or password is None:
                self._logger.error(
                    "Unable to load auth tokens from file, while username and password also not supplied. You need to obtain a login token with your username and password the first time you use the app.")
                return False
            try:
    ...
    def read_transit_key_rsa(self, name, mount_point=None):
        response = self.hvac_client.secrets.transit.read_key(
    ...
    def export_transit_key_rsa(self, name, mount_point=None):
        response = self.hvac_client.secrets.transit.export_key(
            name=name, key_type="encryption-key", mount_point=mount_point)
    ...
    def decrypt_data_key(self, name, ciphertext):
        decrypt_data_response = self.hvac_client.secrets.transit.decrypt_data(
    ...
    def read_secret_metadata(self, path):
        try:
            response = self.hvac_client.secrets.kv.v2.read_secret_metadata(
                path)
